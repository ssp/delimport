...
/*! @function PDEBUG
    ...
    @param args A variable number of arguments suitable for the given 'format' string. */
            ...
            #define PDEBUG(format, ...) /* Do nothing */
                ...
                #define PDEBUG(format, ...) __POUT([@"%s: %s:%d - " stringByAppendingString:format], __FILE__, __func__, __LINE__, ## __VA_ARGS__)
            #else
                #define PDEBUG __POUT
            #endif
        #elif defined(PDEBUG_TO_STDERR)
            #ifdef TAGGED_DEBUGGING
                #define PDEBUG(format, ...) __PERR([@"%s: %s:%d - " stringByAppendingString:format], __FILE__, __func__, __LINE__, ## __VA_ARGS__)
            #else
                #define PDEBUG __PERR
            #endif
        #else
            #ifdef TAGGED_DEBUGGING
                #define PDEBUG(format, ...) __PCONSOLE([@"%s: %s:%d - " stringByAppendingString:format], __FILE__, __func__, __LINE__, ## __VA_ARGS__)
            #else
...
/*! @function PDEBUGC
    ...
    #elif defined(PDEBUG_TO_STDOUT)
        #ifdef TAGGED_DEBUGGING
            #define PDEBUGC(format, ...) __POUTC("%s: %s:%d - " format, __FILE__, __func__, __LINE__, ## __VA_ARGS__)
        #else
            #define PDEBUGC __POUTC
        #endif
    #elif defined(PDEBUG_TO_STDERR)
        #ifdef TAGGED_DEBUGGING
            #define PDEBUGC(format, ...) __PERRC("%s: %s:%d - " format, __FILE__, __func__, __LINE__, ## __VA_ARGS__)
        #else
            #define PDEBUGC __PERRC
        #endif
    #else
        #ifdef TAGGED_DEBUGGING
            #define PDEBUGC(format, ...) __PCONSOLEC("%s: %s:%d - " format, __FILE__, __func__, __LINE__, ## __VA_ARGS__)
        #else
